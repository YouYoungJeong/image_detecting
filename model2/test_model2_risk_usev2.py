...
# ============================================================
# test_model2_risk_from_model1_v2.py
# 모델2 위험도 분류 best.pt 불러와서 test 검증 코드
# model1_v2_best.pt로 생성한 Tank crop 데이터셋 기반 모델2 검증
# VSCode / 로컬 py 실행용
# ============================================================
...
# 모델1 v2 best.pt 경로
# 모델1은 Tank detection/crop에 사용된 모델
...
# 학습 코드와 동일한 모델2 run 이름
MODEL2_RUN_NAME = "model2_risk_from_model1_v2"

# Ultralytics 학습 결과 폴더
MODEL2_RUN_DIR = CLASSIFY_RUNS_DIR / MODEL2_RUN_NAME

# 1순위: weights 폴더에 따로 저장한 최종 모델2 best.pt
FINAL_MODEL2_BEST_PATH = BASE_DIR / "weights" / "model2_risk_from_model1_v2_best.pt"

# 2순위: runs/classify 안에 있는 Ultralytics 기본 best.pt
RUN_MODEL2_BEST_PATH = MODEL2_RUN_DIR / "weights" / "best.pt"

# 실제 테스트에 사용할 모델 경로
MODEL2_BEST_PATH = FINAL_MODEL2_BEST_PATH
...
def check_paths():
    ...
    # model1_v2_best.pt 확인
    # 테스트에 직접 사용하지는 않지만,
    # 현재 모델2가 model1_v2_best.pt 기반 crop 데이터셋으로 만들어졌다는 것을 확인하기 위함
    ...
    # 모델2 best.pt 확인
    # 1순위: weights/model2_risk_from_model1_v2_best.pt
    # 2순위: runs/classify/model2_risk_from_model1_v2/weights/best.pt
    global MODEL2_BEST_PATH

    if FINAL_MODEL2_BEST_PATH.exists():
        MODEL2_BEST_PATH = FINAL_MODEL2_BEST_PATH
    elif RUN_MODEL2_BEST_PATH.exists():
        MODEL2_BEST_PATH = RUN_MODEL2_BEST_PATH
    else:
        raise FileNotFoundError(
            "모델2 best.pt를 찾을 수 없습니다.\n"
            f"1순위 경로: {FINAL_MODEL2_BEST_PATH}\n"
            f"2순위 경로: {RUN_MODEL2_BEST_PATH}"
        )
    ...
    print("MODEL1_V2_BEST_PATH:", MODEL1_V2_BEST_PATH)
...
def save_prediction_visuals():
    ...
    for idx, img_path in enumerate(sorted(test_images), 1):
        ...
        # risk_1_low 관련 이미지만 별도 저장
        # true가 risk_1_low인 경우 또는 pred가 risk_1_low인 경우 모두 포함
    ...
    print("\n전체 test 개수:", len(df))
    print("정답 개수:", int(df["correct"].sum()))
    print("오답 개수:", int((~df["correct"]).sum()))
    print("정확도:", round(float(df["correct"].mean()), 4))

    return df


# ============================================================
# 10. 오답 종류별 4x4 grid 생성
# ============================================================
...
def create_error_type_grids():
    ...
    if not CSV_PATH.exists():
        raise FileNotFoundError(
            f"CSV 파일이 없습니다. 먼저 save_prediction_visuals()를 실행하세요: {CSV_PATH}"
        )
